...
def get_pairwise_homography(img1, img2):
    """Computes Homography H mapping points from img2 to img1 frame."""
...
def stitch_n_images(folder_path):
    ...
    print(f"Loaded {num_imgs} images: {filenames}")

    # Step 1: Compute adjacent pair homographies H_{i, i+1} (transforms i+1 -> i)
    H_adjacent = {}
    valid_mask = [True] * num_imgs

    for i in range(num_imgs - 1):
        name1, name2 = filenames[i], filenames[i+1]
        H, inliers = get_pairwise_homography(images[i], images[i+1])
        
        if H is not None:
            H_adjacent[i] = H
            print(f"[MATCH OK] Pair ({name1} <-> {name2}) connected with {inliers} inliers.")
        else:
            print(f"[REJECT IMAGE] {name2} rejected — insufficient inliers ({inliers}) with {name1}.")
    ...
    # Step 2: Choose Anchor (Middle image of accepted set)
    anchor_idx = N // 2
    print(f"\n---> Selected Reference Anchor: {accepted_names[anchor_idx]} (Index {anchor_idx})")

    # Step 3: Chain Homographies relative to Anchor
    # H_global[i] maps image i into anchor's coordinate frame
    H_global = [None] * N
    H_global[anchor_idx] = np.eye(3, dtype=np.float32)

    # Chain backwards from anchor to index 0
    for i in range(anchor_idx - 1, -1, -1):
        # H_{i -> anchor} = H_{i+1 -> anchor} @ H_{i -> i+1}
        # H_adjacent[i] maps i+1 to i, so inv(H_adjacent[i]) maps i to i+1
    ...
    # Translation matrix to shift output into positive canvas coordinates
    T = np.array([
        [1, 0, -x_min],
        [0, 1, -y_min],
        [0, 0, 1]
    ...
    # Step 5: Warp & Composite all images onto one canvas
    ...
    for i in range(N):
        ...
        # Overlay non-black pixels onto canvas
        mask = (warped > 0)
...
if __name__ == "__main__":
    # Point this to your folder containing 01.jpg, 02.jpg ... 09.jpg
    FOLDER_PATH = "VPG/S01" 
    
    panorama = stitch_n_images(FOLDER_PATH)

    if panorama is not None:
        # Create a resizable window to inspect the high-res panorama
        cv2.namedWindow("N-Image Stitched Panorama", cv2.WINDOW_NORMAL)
        cv2.imshow("N-Image Stitched Panorama", panorama)
        
        print("\nDisplaying panorama. Press any key on the image window to close.")
        cv2.waitKey(0)
